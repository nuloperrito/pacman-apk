...
class GState {
  ...
  static final connectedDevices = SharedValue<List<AdbDevice>>(value: []);
  ...
  static final adbIsStarting = SharedValue<bool>(value: false);
  ...
  static final installTimeout = PersistableValue<int>(value: 90, loader: (o)=>o.timeout, setter: (o,e)=> o.timeout = e);
  ...
  static final versionCode = SharedValue<int>(value: 0);
  static final minSdkVersion = SharedValue<int>(value: 0);
  ...
  static final cancelCallback = SharedValue<Function()?>(value: null);
...
class AppOptions {
  ...
  static Future<Options> _optionsFuture = () async {
    ...
    // TODO options file migration; remove eventually
    final _oldOptionsFile = File("${oldDirectory.path}\\options.bin");
    
    final directory = Directory("${Env.USER_PROFILE}${RegExp(r'.*[/\\]$').hasMatch(Env.USER_PROFILE) ? '' : r'\'}.pacman-apk\\")..createSync();
    
    // TODO options file migration; remove eventually
    if (oldDirectory.existsSync()) {
      if (_oldOptionsFile.existsSync()) {
        if (!File("${directory.path}\\options.bin").existsSync()) _oldOptionsFile.copySync("${directory.path}\\options.bin");
        _oldOptionsFile.deleteSync();
      }
...
class AdbDevice {
  ...
  final bool isWired;
  final bool isWSA;
  AdbDevice(this.id, this.name, this.isWired, this.isWSA);

  @override
  bool operator ==(Object other) => identical(this, other) || other is AdbDevice && id == other.id;
  @override
