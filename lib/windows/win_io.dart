...
base class PROCESSENTRY32 extends Struct {
  @DWORD() external int dwSize;
  @DWORD() external int cntUsage;
  @DWORD() external int th32ProcessID;
  @IntPtr() external int th32DefaultHeapID;
  @DWORD() external int th32ModuleID;
  @DWORD() external int cntThreads;
  @DWORD() external int th32ParentProcessID;
  @LONG() external int pcPriClassBase;
  @DWORD() external int dwFlags;
  @Array(_MAX_PATH) external Array<Uint16> szExeFile;
}

final _CreateToolhelp32Snapshot = kernel32.lookupFunction<
    IntPtr Function(Uint32 dwFlags, Uint32 th32ProcessID),
    int Function(int dwFlags, int th32ProcessID)>('CreateToolhelp32Snapshot');

final _Process32FirstW = kernel32.lookupFunction<
    Int32 Function(IntPtr hSnapshot, Pointer<PROCESSENTRY32> lppe),
    int Function(int hSnapshot, Pointer<PROCESSENTRY32> lppe)>('Process32FirstW');

final _Process32NextW = kernel32.lookupFunction<
    Int32 Function(IntPtr hSnapshot, Pointer<PROCESSENTRY32> lppe),
    int Function(int hSnapshot, Pointer<PROCESSENTRY32> lppe)>('Process32NextW');

enum _FILE_INFO_BY_HANDLE_CLASS {
  FileDispositionInfo(4),
  FileDispositionInfoEx(21);

  const _FILE_INFO_BY_HANDLE_CLASS(this.value);
...
class FileDisposeQueue {
  ...
  bool _setLock(int handle, bool lock) {
    Pointer<BOOL> lpBool = calloc<BOOL>()..value = lock ? TRUE : FALSE;
    try {
...
class WinIO {
  ...
  /// Check if a specific process is currently running using ToolHelp32 Snapshot
  static bool isProcessRunning(String processName) {
    final hSnapshot = _CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);
    ...
    try {
      int result = _Process32FirstW(hSnapshot, pe32);
      while (result != 0) {
        // szExeFile is typically Array<Uint16> in win32 dart package
        // Iterate up to MAX_PATH (260) to find the null terminator
        final chars = <int>[];
        for (int i = 0; i < MAX_PATH; i++) {
          final c = pe32.ref.szExeFile[i];
          if (c == 0) break;
          chars.add(c);
        }
        
        if (String.fromCharCodes(chars).toLowerCase() == processName.toLowerCase()) {
          return true;
        }
