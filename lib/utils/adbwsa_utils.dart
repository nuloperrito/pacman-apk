...
import 'package:pacman_apk/windows/win_pkg.dart';
...
extension ProcessResultTimeout on ProcessResult {
  bool get isTimeout => this.exitCode == -999 && this.stdout == 'TIMEOUT';
}
...
extension ADBUtils on Future<ProcessResult> {
  ...
  Future<ProcessResult> processTimeout(Duration duration) =>
    timeout(duration, onTimeout: () => Future.value(ProcessResult(0, -999, 'TIMEOUT', '')));

  static String get deviceName => '${GState.ipAddress.$}:${GState.androidPort.$}';
  static String _toName(String ipAddress, int port) => port <= 0 ? ipAddress : '$ipAddress:$port';
  ...
  static Future<ProcessResult> devicesLong() => command('devices', param: '-l');
